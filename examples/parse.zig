...
pub fn main() !void {
    ...
    inline for (scalars) |sample| {
        var parsed = try zjson.parseToArena(sample.json, allocator, .{});
        defer parsed.deinit();
        std.debug.print("{s}: {s}\n", .{ sample.label, @tagName(parsed.value) });
    }

    var array_result = try zjson.parseToArena("[1,2,3,4,5]", allocator, .{});
    defer array_result.deinit();
    std.debug.print("array len={d}\n", .{array_result.value.Array.len});

    const object_json = "{\"name\":\"Alice\",\"age\":30}";
    var object_result = try zjson.parseToArena(object_json, allocator, .{});
    ...
    const nested_json =
        ...
        \\}
    ;
    var nested_result = try zjson.parseToArena(nested_json, allocator, .{});
    ...
    var escaped_result = try zjson.parseToArena(escaped_json, allocator, .{});
    defer escaped_result.deinit();
    std.debug.print("escaped: {s}\n", .{escaped_result.value.String});

    var invalid = zjson.parseToArena("{invalid json}", allocator, .{});
    if (invalid) |*parsed| {
        parsed.deinit();
